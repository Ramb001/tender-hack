...
def GeneratePrompts(file_path, request_keys):
    try:
        ...
        prompts['name'] = f"Документ должен содержать упоминание об объекте закупки: *{data['name']}*, часто обозначенных как 'объект закупки'."
        ...
        if contract_cost is not None:
            prompts['cost'] = f"Документ должен содержать явное указание цены контракта: *{contract_cost}*."
        if start_cost is not None:
            prompts['cost'] = f"Документ должен содержать явное указание стартовой цены: *{start_cost}*."
        ...
        for item in data["items"]:
            ...
            prompts['items'].append({"item_name": f"Документ должен содержать упоминание о товаре *{item_name}* с перечислением характеристик:", "characteristics": characteristics})
